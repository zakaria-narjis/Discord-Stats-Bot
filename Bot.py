...
def insert_new_activity(activity:ds.Activity,guild:ds.guild,update:str,member:ds.Member):
...
@tasks.loop(seconds = 10) # repeats after every 10 seconds
                ...
                if member.status!='offline':
                    ...
                    for activity in member.activities:
                        if activity.type.name in ['playing','streaming','listening','watching']:
                            ...
                            insert_new_activity(activity,guild,update,member)
                            doc=collection.find_one({'_id':guild.id,})
            ...
            if  channel.members!=[]:
                for member in channel.members:
                    if member.bot!=True:
                        ...
                        if (vc.self_mute ^ vc.self_deaf) or  all(map(lambda x:not x, [vc.mute,vc.deaf,vc.suppress,vc.afk])):
                ...
                collection.update_one({'_id':guild.id},
                                      {'$set':{'voice_channels.'+str(channel.id)+'.usage':format_timedelta_calc(t0,dt)}})
            else:
                collection.update_one(
                            {'_id':guild.id},
    ...
    if before.bot==False:
        ...
        for activity in after.activities:
            if activity not in before.activities and activity.type.name in ['playing','streaming','listening','watching']:
                update = 'members.' + str(before.id) + '.activities.' + str(activity.type.name) + '.' + activity.name
                insert_new_activity(activity, before.guild, update,after)
                collection.update_one(
                    {'_id': before.guild.id},
                    {'$set': {update+'.previous_check_time':datetime.utcnow()}})
